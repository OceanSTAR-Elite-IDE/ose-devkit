...
import { BOTTOM_CENTER, LEFT_CENTER, NcOverlay, NcOverlayModule, RIGHT_CENTER, TOP_CENTER } from '@oceanstar/components/overlay';
...
@Component({
  imports: [CommonModule, A11yModule, NcOverlayModule],
  selector: 'nc-slider',
  exportAs: 'ncSlider',
  ...
  providers: [{ provide: NcFormFieldControl, useExisting: NcSlider, multi: true }],
})
export class NcSlider implements ControlValueAccessor, OnInit, OnDestroy, NcFormFieldControl<number> {
  tabIndex: number;
  ...
  @ViewChild(NcOverlay, { static: true }) _overlay!: NcOverlay;
  ...
  get _trackFillStyles() {
    ...
    return !this.vertical ? { width: `${sizeValue}%` } : { height: `${sizeValue}%` };
  }
  ...
  get _trackHandleStyles() {
    ...
    if (!this.vertical) {
      return !this.invert ? { left: `${offsetValue}%` } : { right: `${offsetValue}%` };
    } else {
  ...
  get _trackTootipPosition() {
    return !this.vertical ? [TOP_CENTER, BOTTOM_CENTER] : [LEFT_CENTER, RIGHT_CENTER];
  }
  ...
  _trackStepmarkStyles(stepmark: NcSliderStepmark) {
    if (!this.vertical) {
      return !this.invert ? { left: `${stepmark.percent}%` } : { right: `${stepmark.percent}%` };
    } else {
  ...
  private _pointerUp = (event: TouchEvent | MouseEvent) => {
    if (this._isSliding) {
      ...
      if (
        this._valueOnSlideStart !== this.value &&
        !this.disabled &&
        (pointerPositionOnStart?.x !== currentPointerPosition.x || pointerPositionOnStart?.y !== currentPointerPosition.y)
      ) {
        this._emitChangeEvent();
      }
    }
