...
export class NcNotifier implements AfterViewInit {
   ...
   */
  @Output() ready: EventEmitter<NcNotifier> = new EventEmitter<NcNotifier>();
  ...
  get _hasIcon() {
    return this.notification.type !== 'default';
  }
  ...
  shift(distance: number, shiftToMakePlace: boolean): Promise<undefined> {
    return new Promise<undefined>((resolve: (value?: any) => void, reject: () => void) => {
      ...
      if (
        ...
        newElementShift = this._elementShift + distance + this.config.position.vertical.gap;
      } else {
        newElementShift = this._elementShift - distance - this.config.position.vertical.gap;
      }
  ...
  private _getAutoHideable(): boolean {
    return this.config.behaviour.hiddenStrategy === 'content' || this.config.behaviour.hiddenStrategy === 'absoluted';
  }
  ...
  private _setup(): void {
    ...
    if (this.config.position.horizontal.position === 'left') {
      this.renderer.setStyle(this._element, 'left', `${this.config.position.horizontal.distance}px`);
    } else if (this.config.position.horizontal.position === 'right') {
      this.renderer.setStyle(this._element, 'right', `${this.config.position.horizontal.distance}px`);
    } else {
      this.renderer.setStyle(this._element, 'left', '50%');
      // Let's get the GPU handle some work as well (#perfmatters)
      this.renderer.setStyle(this._element, 'transform', 'translate3d( -50%, 0, 0 )');
    }
    if (this.config.position.vertical.position === 'top') {
      this.renderer.setStyle(this._element, 'top', `${this.config.position.vertical.distance}px`);
    } else {
      this.renderer.setStyle(this._element, 'bottom', `${this.config.position.vertical.distance}px`);
    }
