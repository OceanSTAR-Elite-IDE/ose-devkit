 ...
 */
@Component({
  imports: [CommonModule, NcNotifier],
...
export class NcNotifierContainer implements OnDestroy {
  /**
   ...
   */
  onNotificationReady(notificationComponent: NcNotifier): void {
    let currentNotification: NcNotifierRef = this.notifications[this.notifications.length - 1]; // Get the latest notification
  ...
  private _continueHandleShowAction(notification: NcNotifierRef): void {
    ...
    if (numberOfNotifications === 1) {
      ...
      if (this._config.behaviour.stacking === false || this._config.behaviour.stacking < implicitStackingLimit) {
        ...
        if (numberOfNotifications > this._config.behaviour.stacking) {
          const oldNotifications: Array<NcNotifierRef> = this.notifications.slice(1, numberOfNotifications - 1);

          // Are animations enabled?
          if (this._config.animations.enabled) {
            // Is animation overlap enabled?
            if (this._config.animations.overlap !== false && this._config.animations.overlap > 0) {
              ...
              stepPromises.push(
                new Promise<undefined>((resolve: (value?: any) => void, reject: () => void) => {
                  this.notifications[0].component.hide().then(() => {
          ...
          const oldNotifications: Array<NcNotifierRef> = this.notifications.slice(0, numberOfNotifications - 1);
        ...
        Promise.all(stepPromises).then(() => {
          if (numberOfNotifications > (this._config.behaviour.stacking as number)) {
            this.removeNotificationFromList(this.notifications[0]);
          }
  ...
  private _handleHideAction(action: NcNotifierAction): Promise<undefined> {
    return new Promise<undefined>((resolve: (value?: any) => void, reject: () => void) => {
      ...
      const notification: NcNotifierRef | undefined = this.findNotificationById(action.payload);
      ...
      const oldNotifications: Array<NcNotifierRef> = this.notifications.slice(0, notificationIndex);

      // Do older notifications exist, and thus do we need to shift other notifications as a consequence?
      if (oldNotifications.length > 0) {
        // Are animations enabled?
        if (this._config.animations.enabled && this._config.animations.hide.speed > 0) {
          // Is animation overlap enabled?
          if (this._config.animations.overlap !== false && this._config.animations.overlap > 0) {
    ...
    return new Promise<undefined>((resolve: (value?: any) => void, reject: () => void) => {
      ...
      if (
        ...
        for (let i: number = numberOfNotifications - 1; i >= 0; i--) {
          const animationOffset: number = this._config.position.vertical.position === 'top' ? numberOfNotifications - 1 : i;
          setTimeout(() => {
            this.notifications[i].component.hide().then(() => {
              // Are we done here, was this the last notification to be hidden?
              if (
                (this._config.position.vertical.position === 'top' && i === 0) ||
                (this._config.position.vertical.position === 'bottom' && i === numberOfNotifications - 1)
              ) {
   ...
   * @returns Promise, resolved when done
   */
  private shiftNotifications(notifications: Array<NcNotifierRef>, distance: number, toMakePlace: boolean): Promise<undefined> {
    return new Promise<undefined>((resolve: (value?: any) => void, reject: () => void) => {
  ...
  private removeNotificationFromList(notification: NcNotifierRef): void {
    this.notifications = this.notifications.filter((item: NcNotifierRef) => item.component !== notification.component);
    this._changeDetectorRef.markForCheck(); // Run change detection because the notification list changed
  }
   ...
   */
  private findNotificationById(notificationId: string): NcNotifierRef | undefined {
    return this.notifications.find((currentNotification: NcNotifierRef) => currentNotification.id === notificationId);
  }
  ...
  private findNotificationIndexById(notificationId: string): number | undefined {
    const notificationIndex: number = this.notifications.findIndex((currentNotification: NcNotifierRef) => currentNotification.id === notificationId);
