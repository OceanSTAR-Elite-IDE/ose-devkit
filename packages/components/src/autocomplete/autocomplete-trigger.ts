...
import { fromOutsideElementClick, NcOption, NcOptionSelectionChange } from '@oceanstar/components/core';
import { BOTTOM_LEFT, TOP_LEFT } from '@oceanstar/components/overlay';
...
export const NC_AUTOCOMPLETE_VALUE_ACCESSOR: Provider = {
  ...
  useExisting: forwardRef(() => NcAutocompleteTrigger),
  multi: true,
};
...
@Directive({
  ...
  providers: [NC_AUTOCOMPLETE_VALUE_ACCESSOR, NC_AUTOCOMPLETE_SCROLL_STRATEGY_FACTORY_PROVIDER],
})
export class NcAutocompleteTrigger implements ControlValueAccessor, AfterViewInit, OnChanges, OnDestroy {
  ...
  @Input('ncAutocomplete') autocomplete!: NcAutocomplete;

  @Input('ncAutocompleteConnectedTo') connectedTo!: NcAutocompleteOrigin;
  ...
  get panelClosingActions(): Observable<NcOptionSelectionChange | null> {
    ...
    ).pipe(map(event => (event instanceof NcOptionSelectionChange ? event : null)));
  }
  ...
  get activeOption(): NcOption | null {
